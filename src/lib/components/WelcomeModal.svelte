<script>
  ...
  function choose(action) {
    ...
    else if (action === 'ccp') customiser.loadPreset('ccp_default');
  ...
  const OPTIONS = [
    ...
    ['ccp', '🪐', t('welcome.ccp'), t('welcome.ccpDesc')],
    ['import', '⬆️', t('welcome.import'), t('welcome.importDesc')],
    ['blank', '📄', t('welcome.blank'), t('welcome.blankDesc')],
  ];
</script>

<Modal title={t('welcome.title')} onclose={() => { customiser.dismissWelcome(); onclose?.(); }} maxWidth="max-w-lg">
  <p class="text-xs text-app-muted mb-4">{t('welcome.intro')}</p>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
    {#each OPTIONS as [action, icon, label, desc]}
      <button onclick={() => choose(action)} class="text-left bg-app-panel2 border border-app-border hover:border-app-accent rounded-lg p-3 transition-colors">
